...
static atomic_t a_value, b_value;
...
{
    atomic_t *value = (atomic_t *)filp->private_data;
    int v, len;
    char tmp[TMPSIZE];

    if (strcmp(filp->f_path.dentry->d_name.name, "add") == 0)
    {
        v = atomic_read(&a_value) + atomic_read(&b_value);
        len = snprintf(tmp, TMPSIZE, "%d\n", v);
    }
    else if (strcmp(filp->f_path.dentry->d_name.name, "sub") == 0)
    {
        v = atomic_read(&a_value) - atomic_read(&b_value);
        len = snprintf(tmp, TMPSIZE, "%d\n", v);
    }
                              ...
                              size_t count, loff_t *offset)
{
        ...
        else if (strcmp(filp->f_path.dentry->d_name.name, "add") == 0)
        {        len = snprintf(tmp, TMPSIZE, "%d\n", atomic_read(&a_value) + atomic_read(&b_value));
		printk(KERN_INFO "myfs_read_file: add = %d\n", atomic_read(&a_value) + atomic_read(&b_value));
	}
        else if (strcmp(filp->f_path.dentry->d_name.name, "sub") == 0)
                len = snprintf(tmp, TMPSIZE, "%d\n", atomic_read(&a_value) - atomic_read(&b_value));
        else
                               ...
                               size_t count, loff_t *offset)
{
        ...
        if (copy_from_user(tmp, buf, count))
                return -EFAULT;
        if (strcmp(filp->f_path.dentry->d_name.name, "add") != 0 && strcmp(filp->f_path.dentry->d_name.name, "sub") != 0)
                atomic_set(value, simple_strtol(tmp, NULL, 10));
        else
                return -EINVAL;
        return count;
}
...
static void myfs_create_files(struct super_block *sb, struct dentry *root)
{
        ...
        if (inputdir)
        {
                myfs_create_file(sb, inputdir, "a", &a_value);
                myfs_create_file(sb, inputdir, "b", &b_value);
        }
        outputdir = myfs_create_dir(sb, root, "output");
        if (outputdir)
        {
                myfs_create_file(sb, outputdir, "add", NULL);
                myfs_create_file(sb, outputdir, "sub", NULL);
        }
}
